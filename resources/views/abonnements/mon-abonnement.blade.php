    ...
    <div style="background: white; padding: 2rem; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
        ...
        @if($abonnementActif)
            <div style="background: #e3f2fd; padding: 2rem; border-radius: 8px; margin-bottom: 2rem; border-left: 4px solid #1976d2;">
                ...
                <form method="POST" action="{{ route('abonnements.toggle-renouvellement') }}" style="background: white; padding: 1rem; border-radius: 4px; margin-bottom: 1.5rem;">
                    @csrf
                    <div style="display: flex; align-items: center; justify-content: space-between;">
                        <div>
                            <strong style="color: #333; display: block; margin-bottom: 0.25rem;">Renouvellement automatique</strong>
                        ...
                        <label style="position: relative; display: inline-block; width: 60px; height: 34px; cursor: pointer;">
                            ...
                            <span style="position: absolute; top: 0; left: 0; right: 0; bottom: 0; background-color: {{ $abonnementActif->renouvellement_automatique ? '#28a745' : '#ccc' }}; border-radius: 34px; transition: 0.4s;">
                                <span style="position: absolute; content: ''; height: 26px; width: 26px; left: 4px; bottom: 4px; background-color: white; border-radius: 50%; transition: 0.4s; transform: translateX({{ $abonnementActif->renouvellement_automatique ? '26px' : '0' }});"></span>
                            </span>
                        </label>
                    </div>
                </form>
                ...
                @if($abonnementActif->date_fin->isPast())
                    <div style="background: #fff3cd; color: #856404; padding: 1rem; border-radius: 4px; border: 1px solid #ffc107;">
                        ...
                        <a href="{{ route('abonnements.index') }}" style="display: block; margin-top: 0.5rem; color: #856404; text-decoration: underline;">
                            Renouveler mon abonnement
                        </a>
                    </div>
                @elseif($abonnementActif->date_fin->diffInDays(now()) <= 7)
                    <div style="background: #fff3cd; color: #856404; padding: 1rem; border-radius: 4px; border: 1px solid #ffc107;">
                        <strong>⚠ Votre abonnement expire dans {{ $abonnementActif->date_fin->diffInDays(now()) }} jour(s).</strong>
                        <a href="{{ route('abonnements.index') }}" style="display: block; margin-top: 0.5rem; color: #856404; text-decoration: underline;">
                            Renouveler mon abonnement
                        </a>
                    </div>
        ...
        @else
            <div style="background: #fff3cd; color: #856404; padding: 2rem; border-radius: 8px; text-align: center; border: 1px solid #ffc107;">
                ...
                <a href="{{ route('abonnements.index') }}" style="display: inline-block; background: #EF3B2D; color: white; padding: 0.75rem 1.5rem; text-decoration: none; border-radius: 4px; font-weight: 500;">
                    ...
                    @foreach($abonnements as $abonnement)
                        @if(!$abonnementActif || $abonnementActif->abonnement_id !== $abonnement->id)
                            <div style="background: #f8f9fa; padding: 1.5rem; border-radius: 4px; border: 1px solid #dee2e6;">
                                ...
                                <a href="{{ route('abonnements.show', $abonnement) }}" style="display: block; text-align: center; background: #EF3B2D; color: white; padding: 0.5rem; border-radius: 4px; text-decoration: none; font-size: 0.875rem;">
                                    Voir les détails
                                </a>
                            </div>
                        @endif
