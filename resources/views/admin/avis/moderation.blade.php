                    ...
                    <div style="padding: 1.5rem; background: #f8f9fa; border-radius: 8px; border-left: 4px solid #ffc107;">
                        <div style="display: grid; grid-template-columns: 1fr auto; gap: 2rem;">
                            <div>
                                <div style="display: flex; align-items: center; gap: 1rem; margin-bottom: 1rem;">
                                    <div>
                                        ...
                                        <div style="color: #666; font-size: 0.875rem;">
                                            Avis pour : <a href="{{ route('annonces.show', $avis->annonce) }}" style="color: #EF3B2D; text-decoration: none;">{{ $avis->annonce->titre }}</a>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <p style="color: #666; line-height: 1.6; margin: 0 0 1rem 0; background: white; padding: 1rem; border-radius: 4px;">{{ $avis->commentaire }}</p>

                                @if($avis->photos && count($avis->photos) > 0)
                                    <div style="display: flex; gap: 0.5rem; flex-wrap: wrap; margin-bottom: 1rem;">
                                        @foreach($avis->photos as $photoPath)
                                            <img src="{{ asset('storage/' . $photoPath) }}" alt="Photo avis" style="width: 100px; height: 100px; object-fit: cover; border-radius: 4px;">
                                        @endforeach
                                    </div>
                                @endif
                            ...
                            <div style="display: flex; flex-direction: column; gap: 0.5rem;">
                                <form method="POST" action="{{ route('admin.avis.approve', $avis) }}" style="display: inline;">
                                    @csrf
                                    <button type="submit" style="background: #28a745; color: white; border: none; padding: 0.75rem 1.5rem; border-radius: 4px; font-weight: 500; cursor: pointer; white-space: nowrap;">
                                        ✓ Approuver
                                    </button>
                                </form>
