            ...
            <div style="display: flex; flex-direction: column; gap: 25px;">
                ...
                <div class="amazon-card">
                    ...
                    <div style="display: flex; flex-direction: column; gap: 20px;">
                        ...
                        <div id="filters-container" style="{{ $banner->category_id ? 'display: block;' : 'display: none;' }} border-top: 1px solid #eee; padding-top: 20px;">
                            ...
                            <div id="filters-list" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 15px; background: #fcfcfc; border: 1px solid #eee; padding: 15px; border-radius: 4px;">
                                @if($banner->category_id)
                                    @php 
                                        $categoryFilters = \App\Models\CategoryFilter::where('category_id', $banner->category_id)->get();
                                        $selectedFilters = $banner->filters ?? [];
                                    @endphp
                                    @foreach($categoryFilters as $filter)
                                        <div style="margin-bottom: 10px;">
                                            <p style="font-size: 0.8rem; font-weight: 700; color: #555; margin-bottom: 5px; border-bottom: 1px solid #efefef; padding-bottom: 3px;">{{ $filter->nom }}</p>
                                                ...
                                                <div style="display: flex; flex-direction: column; gap: 5px;">
                                                    @foreach($filter->options as $opt)
                                                        <label style="display: flex; align-items: center; gap: 8px; font-size: 0.75rem; cursor: pointer;">
                                                            <input type="checkbox" name="filters[{{ $filter->slug }}][]" value="{{ $opt }}" 
                                                                {{ isset($selectedFilters[$filter->slug]) && in_array($opt, $selectedFilters[$filter->slug]) ? 'checked' : '' }}
                                                                style="accent-color: #e47911;">
                                                            {{ $opt }}
                                                        </label>
                                                    @endforeach
                                                </div>
                                            @else
                                                <p style="font-size: 0.7rem; color: #999;">Pas d'options</p>
                                            @endif
                                        </div>
                                    @endforeach
                                @endif
...
// Initial check for N3 listener
document.addEventListener('DOMContentLoaded', function() {
    const n3Select = document.getElementById('category_id');
    if (n3Select && !n3Select.dataset.listenerAdded) {
        n3Select.addEventListener('change', function() {
            loadFilters(this.value);
        });
        n3Select.dataset.listenerAdded = "true";
