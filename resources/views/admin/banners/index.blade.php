    ...
    <div style="max-width: 100%;">
        ...
        <div style="background: #fff; border: 1px solid #eee; border-radius: 2px; padding: 1.5rem;">
            ...
            <div style="margin-bottom: 0.5rem;">
                <h1 style="font-size: 1.25rem; font-weight: 700; color: #333; margin: 0;">Gestion des Bannières</h1>
            </div>
            ...
            <div style="display: flex; gap: 12px; align-items: center; margin-bottom: 1.5rem;">
                <a href="{{ route('admin.banners.create') }}" style="display: flex; align-items: center; gap: 8px; background: #e67e00; color: #fff; padding: 8px 16px; border-radius: 6px; font-size: 0.85rem; font-weight: 500; text-decoration: none; transition: opacity 0.2s;" onmouseover="this.style.opacity='0.8'" onmouseout="this.style.opacity='1'">
            ...
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem; flex-wrap: wrap; gap: 10px;">
                ...
                <div style="font-size: 0.85rem; color: #333;">
                    <form action="{{ route('admin.banners.index') }}" method="GET" style="display: flex; align-items: center;">
            ...
            <table style="width: 100%; border-collapse: collapse; border: 1px solid #eee;">
                <thead>
                    <tr style="background: #fff; border-bottom: 2px solid #eee;">
                        <th style="padding: 10px; border: 1px solid #eee; text-align: center; font-size: 0.82rem; font-weight: 700; color: #333; width: 60px;">Ordre</th>
                        <th style="padding: 10px; border: 1px solid #eee; text-align: center; font-size: 0.82rem; font-weight: 700; color: #333; width: 120px;">Visuel</th>
                        <th style="padding: 10px; border: 1px solid #eee; text-align: left; font-size: 0.82rem; font-weight: 700; color: #333;">Détails de la Bannière</th>
                        <th style="padding: 10px; border: 1px solid #eee; text-align: left; font-size: 0.82rem; font-weight: 700; color: #333; width: 180px;">Période</th>
                    ...
                    @forelse($banners as $banner)
                        <tr style="transition: background 0.1s;" onmouseover="this.style.background='#f9f9f9'" onmouseout="this.style.background='transparent'">
                            ...
                            <td style="padding: 10px; border: 1px solid #eee; text-align: center;">
                                <img src="{{ $banner->image_url }}" alt="{{ $banner->title }}" style="width: 80px; height: 32px; object-fit: cover; border-radius: 4px; border: 1px solid #eee;">
                            </td>
                            <td style="padding: 10px; border: 1px solid #eee; font-size: 0.85rem; color: #333;">
                                <div style="font-weight: 600; color: #333; margin-bottom: 2px;">{{ $banner->title }}</div>
                                @if($banner->link_url)
                                    <div style="font-size: 0.75rem; color: #2563eb; font-weight: 500;">
                                        <i class="fas fa-link" style="font-size: 0.7rem; opacity: 0.7;"></i> {{ Str::limit($banner->link_url, 40) }}
                                    </div>
                                @endif
                            </td>
                            <td style="padding: 10px; border: 1px solid #eee; font-size: 0.8rem; color: #64748b; font-weight: 500;">
                                @if($banner->start_date || $banner->end_date)
                                    <div style="display: flex; flex-direction: column; gap: 2px;">
                                        <span>Du : {{ $banner->start_date ? $banner->start_date->format('d/m/Y') : '∞' }}</span>
                                        <span>Au : {{ $banner->end_date ? $banner->end_date->format('d/m/Y') : '∞' }}</span>
                                    </div>
                                @else
                            ...
                            <td style="padding: 10px; border: 1px solid #eee;">
                                ...
                                @else
                                    <span style="background: #fff5f5; color: #c53030; padding: 2px 8px; border-radius: 12px; font-size: 0.75rem; font-weight: 600;">Inactive</span>
                                @endif
                            </td>
                            <td style="padding: 10px; border: 1px solid #eee; text-align: right;">
                                <div style="display: flex; gap: 6px; justify-content: flex-end;">
                                    ...
                                    <form id="toggle-form-{{ $banner->id }}" action="{{ route('admin.banners.toggle-status', $banner) }}" method="POST" style="display:inline;">
                                        @csrf @method('PATCH')
                                        <button type="button" onclick="confirmToggle({{ $banner->id }}, {{ $banner->active ? 'true' : 'false' }})" 
                                                style="display: flex; align-items: center; justify-content: center; width: 32px; height: 32px; background: {{ $banner->active ? '#fff7ed' : '#f0fdf4' }}; color: {{ $banner->active ? '#c2410c' : '#15803d' }}; border-radius: 6px; font-size: 0.8rem; border: 1px solid {{ $banner->active ? '#ffedd5' : '#dcfce7' }}; cursor: pointer; transition: all 0.2s;" 
                                                onmouseover="this.style.background='{{ $banner->active ? '#ffedd5' : '#dcfce7' }}'" onmouseout="this.style.background='{{ $banner->active ? '#fff7ed' : '#f0fdf4' }}'"
                                                title="{{ $banner->active ? 'Désactiver' : 'Activer' }}">
                                            <i class="fas fa-{{ $banner->active ? 'lock' : 'lock-open' }}"></i>
                                        </button>
                                    </form>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" style="padding: 3rem; text-align: center; color: #94a3b8; font-size: 0.9rem;">
                                <i class="fas fa-image" style="font-size: 2rem; display: block; margin-bottom: 1rem; opacity: 0.3;"></i>
            ...
            @if($banners->total() > 0)
                <div style="border-top: 1px solid #f3f3f3; margin-top: 1.5rem; padding-top: 1.5rem; display: flex; justify-content: space-between; align-items: center;">
                    <div style="font-size: 0.85rem; color: #64748b; font-weight: 500;">
                        ligne {{ $banners->firstItem() ?? 0 }} sur {{ $banners->total() }}
                    </div>
                    <div style="display: flex; border: 1px solid #e2e8f0; border-radius: 6px; overflow: hidden; background: #fff;">
                        ...
                        @else
                            <a href="{{ $banners->previousPageUrl() }}" style="padding: 8px 16px; background: #fff; color: #2563eb; text-decoration: none; font-size: 0.85rem; border-right: 1px solid #e2e8f0; transition: all 0.2s; font-weight: 500;" onmouseover="this.style.background='#f8fafc'">Prec</a>
                        @endif
                        ...
                        @if($banners->hasMorePages())
                            <a href="{{ $banners->nextPageUrl() }}" style="padding: 8px 16px; background: #fff; color: #2563eb; text-decoration: none; font-size: 0.85rem; transition: all 0.2s; font-weight: 500; border-left: 1px solid #e2e8f0;" onmouseover="this.style.background='#f8fafc'">Suiv</a>
                        @else
        ...
        function confirmToggle(id, isActive) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    // Create a virtual form and submit it to the toggleStatus route
                    const form = document.getElementById('toggle-form-' + id);
                    // Standard POST request via form submit
                    form.submit();
                }
            })
        }
