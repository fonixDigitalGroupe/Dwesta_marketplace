...
<div style="max-width: 800px; margin: 3rem auto; padding: 2rem;">
    <div style="background: white; padding: 2rem; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
        <div style="margin-bottom: 2rem;">
            ...
            <h1 style="color: #333; margin-top: 0;">Créer une Catégorie</h1>
        </div>
        ...
        <form method="POST" action="{{ route('admin.categories.store') }}">
            ...
            <div style="margin-bottom: 1.5rem;">
                ...
                <input type="text" name="nom" value="{{ old('nom') }}" required style="width: 100%; padding: 0.75rem; border: 1px solid #ddd; border-radius: 4px; font-size: 1rem;">
                @error('nom')
            ...
            <div style="margin-bottom: 1.5rem;">
                ...
                <select name="parent_id" style="width: 100%; padding: 0.75rem; border: 1px solid #ddd; border-radius: 4px; font-size: 1rem;">
                    ...
                    @php
                        function formatCategoryOption($category, $level = 0) {
                    ...
                    @foreach($categoryTree as $item)
                        <option value="{{ $item['category']->id }}" {{ old('parent_id') == $item['category']->id ? 'selected' : '' }}>
                            {{ formatCategoryOption($item['category'], $item['level']) }}
                        </option>
                    @endforeach
                </select>
                <small style="color: #666; font-size: 0.875rem;">Sélectionnez une catégorie parente pour créer une sous-catégorie (une sous-catégorie peut aussi être parent)</small>
                @error('parent_id')
                    <span style="color: #EF3B2D; font-size: 0.875rem;">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div style="margin-bottom: 1.5rem;">
                ...
                <textarea name="description" rows="4" style="width: 100%; padding: 0.75rem; border: 1px solid #ddd; border-radius: 4px; font-size: 1rem;">{{ old('description') }}</textarea>
                @error('description')
                    <span style="color: #EF3B2D; font-size: 0.875rem;">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div style="margin-bottom: 1.5rem;">
                ...
                <input type="text" name="icone" value="{{ old('icone') }}" placeholder="shopping-bag, car, home, etc." style="width: 100%; padding: 0.75rem; border: 1px solid #ddd; border-radius: 4px; font-size: 1rem;">
            ...
            <div style="margin-bottom: 1.5rem;">
                ...
                <input type="number" name="ordre" value="{{ old('ordre', 0) }}" min="0" style="width: 100%; padding: 0.75rem; border: 1px solid #ddd; border-radius: 4px; font-size: 1rem;">
            ...
            <div style="margin-bottom: 2rem;">
                <label style="display: flex; align-items: center; color: #333; cursor: pointer;">
                    <input type="checkbox" name="actif" value="1" {{ old('actif', true) ? 'checked' : '' }} style="margin-right: 0.5rem;">
