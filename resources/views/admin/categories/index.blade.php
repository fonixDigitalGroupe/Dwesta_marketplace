    ...
    <div style="max-width: 100%;">
        ...
        <div
            ...
            <div
                ...
                <form action="{{ route('admin.categories.l' . $level) }}" method="GET"
            ...
            <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px; border: 1px solid #e7e7e7;">
                <thead>
                    <tr style="background: #f6f6f6; border-bottom: 1px solid #e7e7e7;">
                        @if($level == 1)
                            <th
                                style="padding: 10px 15px; text-align: center; font-size: 0.75rem; font-weight: 700; color: #111; text-transform: uppercase; border-right: 1px solid #e7e7e7; width: 60px;">
                                Icône</th>
                        @endif
                        <th
                        ...
                        @if($level == 1)
                            <th
                                style="padding: 10px 15px; text-align: left; font-size: 0.75rem; font-weight: 700; color: #111; text-transform: uppercase; border-right: 1px solid #e7e7e7; width: 140px;">
                                Famille</th>
                        @endif
                        <th
                            style="padding: 10px 15px; text-align: right; font-size: 0.75rem; font-weight: 700; color: #111; text-transform: uppercase; width: 200px;">
                            Actions</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse($categories as $category)
                        <tr style="border-bottom: 1px solid #e7e7e7; transition: background 0.1s;"
                            ...
                            @if($level == 1)
                                <td
                                    style="padding: 12px 15px; text-align: center; font-size: 1.2rem; border-right: 1px solid #e7e7e7;">
                                    {!! $category->icone ?? '📦' !!}
                                </td>
                            @endif
                                ...
                                <td style="padding: 12px 15px; font-size: 0.85rem; color: #555; border-right: 1px solid #e7e7e7;">
                                    ...
                                    @else
                                        <div style="display: flex; flex-direction: column;">
                                            ...
                                            <span
                                                style="font-size: 0.7rem; color: #999;">({{ $category->parent->parent->nom ?? '' }})</span>
                                        </div>
                                    @endif
                                </td>
                            @endif
                            ...
                            @if($level == 1)
                                <td style="padding: 12px 15px; font-size: 0.85rem; color: #555; border-right: 1px solid #e7e7e7;">
                                    @if($category->famille)
                                        <span
                                            style="font-weight: 700; color: #c45500; font-size: 0.75rem; text-transform: uppercase;">{{ $category->famille }}</span>
                                    @else
                    ...
                    @empty
                        <tr>
                            <td colspan="{{ $level == 1 ? 5 : ($level == 2 ? 4 : 5) }}"
            ...
            @if($categories->total() > 0)
                <div
                    ...
                    <div style="font-size: 0.8rem; color: #64748b; font-weight: 500;">
                        Affichage de {{ $categories->firstItem() ?? 0 }} à {{ $categories->lastItem() ?? 0 }} sur
