        ...
        <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid #eff3f6; padding-bottom: 15px; margin-bottom: 24px;">
            <div style="display: flex; align-items: center; gap: 12px;">
                <div style="display: flex; align-items: center; gap: 8px; color: #475569; font-size: 0.75rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.05em; height: 28px;">
                    ...
                    <span>Modifier : {{ $country->name }}</span>
                </div>
            </div>
            ...
            <a href="{{ route('admin.countries.index') }}" class="btn-amazon-secondary" style="width: auto !important; height: 32px !important; padding: 0 16px !important; font-size: 0.8rem;">
                <i class="fas fa-undo" style="color: #ff9900;"></i> Retour à la liste
            </a>
        </div>
            ...
            <div style="display: grid; grid-template-columns: 1fr 380px; gap: 20px; align-items: start;">
                ...
                <div style="display: flex; flex-direction: column; gap: 20px;">
                    ...
                    <div class="amazon-card" style="margin: 0;">
                        <h3 class="section-title">Sélection Rapide (Optionnel)</h3>
                        <div style="margin-bottom: 15px;">
                            <label class="field-label">Réinitialiser via la liste d'Afrique</label>
                            <select id="country-selector">
                                <option value="">-- Conserver les données actuelles --</option>
                    ...
                    <div class="amazon-card" style="margin: 0;">
                        ...
                        <input type="hidden" name="flag" id="flag" value="{{ $country->flag }}">
                        <input type="hidden" name="map_external_url" id="map_external_url">

                        <div style="margin-bottom: 15px;">
                            <label for="name" class="field-label">Nom officiel du pays <small style="color: red;">*</small></label>
                        ...
                        <div style="margin-top: 15px;">
                            ...
                            <input type="text" name="phone_format" id="phone_format" value="{{ old('phone_format', $country->phone_format) }}">
                        </div>
                    </div>
                ...
                <div style="display: flex; flex-direction: column; gap: 20px;">
                    ...
                    <div class="amazon-card" style="margin: 0;">
                        ...
                        <div id="map-status" class="map-status">Localisation en cours...</div>
                    </div>
...
<script>
    ...
    function updateMap(name, lat, lng) {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        let initialLat = 10;
        let initialLng = 0;
        let currentName = "{{ $country->name }}";

        // Attempt to get initial lat/lng from the selected option (if any)
        const selectedOpt = selector.querySelector('option[selected]');
        if (selectedOpt) {
            initialLat = parseFloat(selectedOpt.getAttribute('data-lat'));
            initialLng = parseFloat(selectedOpt.getAttribute('data-lng'));
        }

        map = L.map('interactive-map').setView([initialLat, initialLng], 5);
        L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
            attribution: '© OpenStreetMap'
        }).addTo(map);

        updateMap(currentName, initialLat, initialLng);
        ...
        selector.addEventListener('change', function() {
            ...
            const emoji = selectedOpt.getAttribute('data-emoji');
            ...
            formatInput.value = selectedOpt.getAttribute('data-format');
