                ...
                <tbody>
                    @forelse($livreurs as $livreur)
                        <tr style="border-bottom: 1px solid #e7e7e7; transition: background 0.1s;"
                            ...
                            <td style="padding: 12px 15px; font-size: 0.85rem; color: #555; border-right: 1px solid #e7e7e7;">
                                <div style="font-weight: 500; color: #111;">{{ $livreur->type_document }}</div>
                                <code style="font-family: monospace; background: #f8fafc; padding: 2px 6px; border-radius: 4px; font-size: 0.8rem; color: #475569; border: 1px solid #e2e8f0; margin-top: 4px; display: inline-block;">
                                    {{ $livreur->numero_document ?? 'N/A' }}
                                </code>
                            </td>
                            <td style="padding: 12px 15px; text-align: center; border-right: 1px solid #e7e7e7;">
                                @php
                                    $status = match($livreur->statut_verification) {
            ...
            @if($livreurs->total() > 0)
                <div style="display: flex; justify-content: space-between; align-items: center; padding: 12px 16px; background: #ffffff; border: 1px solid #e2e8f0; border-radius: 0; margin-top: 10px;">
                    <div style="font-size: 0.8rem; color: #64748b; font-weight: 500;">
                        Affichage de {{ $livreurs->firstItem() }} à {{ $livreurs->lastItem() }} sur {{ $livreurs->total() }} résultats
                    </div>
                    <div style="display: flex; border: 1px solid #adb1b8; border-radius: 0; overflow: hidden; box-shadow: 0 1px 2px rgba(0,0,0,0.05); background: #fff;">
                        ...
                        @else
                            <a href="{{ $livreurs->previousPageUrl() }}" style="padding: 6px 12px; background: #fff; color: #111; font-size: 0.8rem; text-decoration: none; border-right: 1px solid #adb1b8;">Précédent</a>
                        @endif

                        @php
                            $startPage = max(1, $livreurs->currentPage() - 2);
                            $endPage = min($livreurs->lastPage(), $startPage + 4);
                        @endphp
                        ...
                        @for ($i = $startPage; $i <= $endPage; $i++)
                            ...
                            @else
                                <a href="{{ $livreurs->url($i) }}" style="padding: 6px 12px; background: #fff; color: #555; font-size: 0.8rem; text-decoration: none; border-right: 1px solid #adb1b8;">{{ $i }}</a>
                            @endif
                        @endfor

                        @if ($livreurs->hasMorePages())
                            <a href="{{ $livreurs->nextPageUrl() }}" style="padding: 6px 12px; background: #fff; color: #111; font-size: 0.8rem; text-decoration: none;">Suivant</a>
                        @else
