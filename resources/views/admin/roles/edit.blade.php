...
@section('content')
    <div style="max-width: 1200px; margin: 0 auto;">
        ...
        @if(in_array($role->name, ['admin', 'vendeur', 'client']))
            <div
                style="background-color: #fffbeb; border: 1px solid #fef3c7; color: #111; padding: 12px 16px; margin-bottom: 20px; border-radius: 4px; font-size: 0.85rem; display: flex; gap: 12px; align-items: center;">
                <i class="fas fa-info-circle" style="color: #e77600; font-size: 1.1rem;"></i>
                <div>
                    <strong>Note :</strong> Ce rôle est un rôle système critique. Vous ne pouvez pas modifier son nom ou son
                    statut, uniquement ses permissions.
                </div>
            </div>
        @endif
            ...
            <div style="display: grid; grid-template-columns: 1fr 2fr; gap: 20px;">
                ...
                <div class="amazon-card">
                    ...
                    <div style="margin-bottom: 20px;">
                        <label for="name"
                            style="display: block; font-size: 0.85rem; font-weight: 700; color: #111; margin-bottom: 5px;">
                            Nom du rôle
                        </label>
                        <input type="text" name="name" id="name" value="{{ old('name', $role->name) }}" required
                            @if(in_array($role->name, ['admin', 'vendeur', 'client'])) readonly
                                style="width: 100%; padding: 8px 12px; border: 1px solid #e7e7e7; border-radius: 3px; font-size: 0.85rem; background: #f9f9f9; color: #777; cursor: not-allowed; outline: none;"
                            @else
                                style="width: 100%; padding: 8px 12px; border: 1px solid #adb1b8; border-radius: 3px; font-size: 0.85rem; outline: none;"
                            @endif>
                        @error('name') <p style="color: #c40000; font-size: 0.75rem; margin-top: 5px;">{{ $message }}</p>
                        @enderror
                    </div>

                    <div style="margin-bottom: 20px;">
                        <label class="checkbox-container"
                            style="@if(in_array($role->name, ['admin', 'vendeur', 'client'])) opacity: 0.6; cursor: not-allowed; @endif">
                ...
                <div class="amazon-card">
                    <div
                        style="display: flex; justify-content: space-between; align-items: baseline; margin-bottom: 20px; border-bottom: 1px solid #e7e7e7; padding-bottom: 10px;">
                        <h3 style="font-size: 1.1rem; font-weight: 500; color: #111; margin: 0;">Permissions accordées</h3>
                        <div style="display: flex; gap: 15px;">
                            <button type="button"
                                onclick="document.querySelectorAll('.perm-checkbox').forEach(cb => cb.checked = true)"
                            ...
                            <button type="button"
                                onclick="document.querySelectorAll('.perm-checkbox').forEach(cb => cb.checked = false)"
                                style="background: none; border: none; color: #0066c0; font-size: 0.8rem; cursor: pointer; padding: 0;"
                                onmouseover="this.style.textDecoration='underline'"
                                onmouseout="this.style.textDecoration='none'">
                                Tout décocher
