        ...
        <div
            ...
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
                ...
                <div style="display: flex; gap: 8px;">
                    <a href="{{ route('admin.roles.create') }}"
                        style="background: linear-gradient(180deg, #007bff 0%, #0056b3 100%); border: 1px solid #004aad; color: #fff; padding: 6px 14px; border-radius: 3px; font-size: 0.8rem; font-weight: 400; text-decoration: none; box-shadow: 0 1px 0 rgba(255,255,255,.4) inset; display: flex; align-items: center; gap: 6px;">
            ...
            <div
                ...
                <div style="display: flex; align-items: center; gap: 8px; font-size: 0.8rem; color: #555;">
                    <span>Afficher</span>
                    <select
                        onchange="window.location.href = '{{ route('admin.roles.index') }}?per_page=' + this.value + '&search={{ $search }}'"
                ...
                <div style="font-size: 0.8rem;">
                    <form action="{{ route('admin.roles.index') }}" method="GET"
                        ...
                        <input type="text" name="search" value="{{ $search }}" placeholder="Nom du rôle..."
                ...
                <thead>
                    <tr style="background: #f6f6f6; border-bottom: 1px solid #e7e7e7;">
                        ...
                        <th
                            ...
                            Permissions</th>
                        <th
                            style="padding: 10px 15px; text-align: center; font-size: 0.75rem; font-weight: 700; color: #111; text-transform: uppercase; width: 100px; border-right: 1px solid #e7e7e7;">
                            Type</th>
                        <th
                            style="padding: 10px 15px; text-align: right; font-size: 0.75rem; font-weight: 700; color: #111; text-transform: uppercase; width: 150px;">
                            Actions</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse($roles as $role)
                        <tr style="border-bottom: 1px solid #e7e7e7; transition: background 0.1s;"
                            ...
                            <td style="padding: 12px 15px; font-size: 0.85rem; color: #0066c0; font-weight: 500; text-transform: capitalize; border-right: 1px solid #e7e7e7; cursor: pointer;"
                            ...
                            <td style="padding: 12px 15px; border-right: 1px solid #e7e7e7;">
                                <div style="display: flex; flex-wrap: wrap; gap: 4px; max-width: 450px;">
                                    ...
                                    @foreach($displayPerms as $perm)
                                        <span
                                            style="font-size: 0.7rem; background: #fff; color: #555; padding: 2px 6px; border-radius: 2px; border: 1px solid #ddd; white-space: nowrap;">
                                            {{ $labels[$perm] ?? $perm }}
                                        </span>
                                    @endforeach
                                    @if($remainingCount > 0)
                                        <span style="font-size: 0.7rem; color: #ff8c00; font-weight: 600; cursor: help;"
                                            title="{{ implode(', ', array_slice($perms, $displayLimit)) }}">
                            ...
                            <td
                                style="padding: 12px 15px; text-align: center; font-size: 0.8rem; color: #555; border-right: 1px solid #e7e7e7;">
                    ...
                    @empty
                        <tr>
                            <td colspan="4" style="padding: 40px; text-align: center; color: #555; font-size: 0.85rem;">Aucun
                                rôle trouvé.</td>
                        </tr>
                    @endforelse
                </tbody>
            ...
            <div
                ...
                <div style="font-size: 0.8rem; color: #64748b; font-weight: 500;">
                    Affichage de {{ $roles->firstItem() ?? 0 }} à {{ $roles->lastItem() ?? 0 }} sur {{ $roles->total() }}
                    résultats
                </div>
                <div
