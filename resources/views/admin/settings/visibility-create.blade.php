...
@section('title', 'Ajouter une Option de Visibilité')
...
<div style="max-width: 1000px; margin: 0 auto;">
    ...
    <div style="background: #fff; border: 1px solid #eff3f6; border-top: none; border-radius: 0 0 8px 8px; box-shadow: 0 10px 25px rgba(0,0,0,0.02); padding: 24px;">
        <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid #eff3f6; padding-bottom: 15px; margin-bottom: 24px;">
            <div style="display: flex; align-items: center; gap: 12px;">
                <div style="display: flex; align-items: center; gap: 8px; color: #475569; font-size: 0.75rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.05em; height: 28px;">
                    ...
                    <span>Nouvelle Option de Visibilité</span>
                </div>
            </div>
        ...
        <form action="{{ route('admin.settings.visibility.store') }}" method="POST">
            @csrf
                    ...
                    <div class="amazon-card" style="margin: 0;">
                        ...
                        <div style="margin-bottom: 20px;">
                            ...
                            <select name="service_type" required>
                                ...
                                <option value="mise_en_avant" {{ old('service_type') == 'mise_en_avant' ? 'selected' : '' }}>Mise en avant</option>
                                <option value="boost" {{ old('service_type') == 'boost' ? 'selected' : '' }}>Boost visibilité</option>
                                <option value="video" {{ old('service_type') == 'video' ? 'selected' : '' }}>Ajout Vidéo</option>
                            </select>
                        </div>

                        <div style="margin-bottom: 0;">
                            <label class="field-label">Description <small style="color: red;">*</small></label>
                            <textarea name="description" rows="5" required placeholder="Décrivez les avantages de ce service pour l'utilisateur...">{{ old('description') }}</textarea>
                        </div>
                    </div>
                </div>
                ...
                <div style="display: flex; flex-direction: column; gap: 20px;">
                    ...
                    <div class="amazon-card" style="margin: 0;">
                        ...
                        <div style="margin-bottom: 15px;">
                            ...
                            <input type="number" name="credits_requis" value="{{ old('credits_requis', 0) }}" min="0" required>
                        </div>

                        <div style="margin-bottom: 15px;">
                            <label class="field-label">Durée (Jours)</label>
                            <input type="number" name="duree_jours" value="{{ old('duree_jours') }}" min="1" placeholder="Permanent">
                        </div>
