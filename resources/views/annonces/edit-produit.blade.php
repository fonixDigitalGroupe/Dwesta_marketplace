    ...
    <div class="create-annonce-container">
        ...
        <aside class="progress-sidebar">
            ...
            <div class="progress-step" data-step="6">
                <div class="step-circle"></div>
                <div class="step-content">
                    <div class="step-number">ETAPE 6</div>
        ...
        <main class="form-content">
            <form id="createAnnonceForm" method="POST" action="{{ route('annonces.update', $annonce) }}" enctype="multipart/form-data">
                ...
                <div class="form-step active" id="step1">
                    ...
                    <div class="form-instructions">
                        <p class="instruction-text">
                            Indiquez quelques <strong>caractéristiques</strong> de votre produit pour faciliter sa recherche dans notre catalogue.
                        </p>
                        <p class="instruction-text">
                            Ne précisez pas l'état de votre produit, cette information vous sera demandée ultérieurement.
                        </p>
                    </div>
                ...
                <div class="form-step" id="step2">
                    ...
                    <div class="form-instructions">
                        <p class="instruction-text">
                            Décrivez les caractéristiques principales de votre produit.
                        </p>
                    </div>
                    
                    <div class="form-group">
                        <label for="categorie_id" class="form-label">Catégorie <span style="color: #193D23;">*</span></label>
                        <select id="categorie_id" name="categorie_id" class="form-input" required>
                            <option value="">Sélectionnez une catégorie</option>
                            @foreach($categories as $categorie)
                                <option value="{{ $categorie->id }}" {{ old('categorie_id', $annonce->categorie_id) == $categorie->id ? 'selected' : '' }}>{{ $categorie->nom }}</option>
                                @if($categorie->enfantsActifs && $categorie->enfantsActifs->count() > 0)
                                    @foreach($categorie->enfantsActifs as $enfant)
                                        <option value="{{ $enfant->id }}" {{ old('categorie_id', $annonce->categorie_id) == $enfant->id ? 'selected' : '' }}>&nbsp;&nbsp;→ {{ $enfant->nom }}</option>
                                    @endforeach
                                @endif
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        <label for="prix" class="form-label">Prix (€) <span style="color: #193D23;">*</span></label>
                        <input 
                            type="number" 
                            id="prix" 
                            name="prix" 
                    ...
                    <div class="form-group">
                        <label for="prix_moyen_marche" class="form-label">Prix moyen du marché (€)</label>
                        <input 
                            type="number" 
                        ...
                        <small style="color: #666; font-size: 0.875rem; margin-top: 0.5rem; display: block;">
                            Prix moyen du marché pour référence (optionnel)
                        </small>
                    </div>
                ...
                <div class="form-step" id="step3">
                    ...
                    <div class="form-instructions">
                        <p class="instruction-text">
                            Décrivez votre produit en détail pour attirer les acheteurs.
                        </p>
                    </div>
                    ...
                    <div class="form-group">
                        <label class="form-label">Photos du produit <span style="color: #193D23;">*</span></label>
                        ...
                        @if($annonce->medias && $annonce->medias->count() > 0)
                            <div class="image-preview-container" id="existingImagesContainer" style="margin-bottom: 1rem;">
                                @foreach($annonce->medias as $media)
                                    <div class="image-preview-item {{ $media->est_principale ? 'main' : '' }}" data-media-id="{{ $media->id }}">
                                        <img src="{{ Storage::url($media->chemin) }}" alt="Photo existante">
                                        <div class="image-preview-actions">
                                            @if(!$media->est_principale)
                                                <button type="button" class="image-preview-btn" onclick="setMainExistingImage({{ $media->id }})" title="Définir comme photo principale">⭐</button>
                                            @endif
                                            <button type="button" class="image-preview-btn" onclick="removeExistingImage({{ $media->id }})" title="Supprimer">×</button>
                                        </div>
                                        @if($media->est_principale)
                                            <div class="image-preview-main-badge">Principale</div>
                                        @endif
                                    </div>
                                @endforeach
                            </div>
                        @endif
                        <div class="image-upload-area" id="imageUploadArea" onclick="document.getElementById('photosInput').click()">
                            <svg class="image-upload-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                            </svg>
                            <div class="image-upload-text">Cliquez pour ajouter des photos ou glissez-déposez</div>
                            <div class="image-upload-hint">1-8 photos, formats JPG, PNG, WEBP (max 5 Mo chacune)</div>
                        </div>
                        <input 
                            type="file" 
                            id="photosInput" 
                ...
                <!-- Étape 4: Caractéristiques détaillées -->
                <div class="form-step" id="step4">
                    ...
                    <div class="form-instructions">
                        <p class="instruction-text">
                            Ajoutez des détails supplémentaires sur votre produit pour aider les acheteurs.
                        </p>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input 
                            ...
                            value="{{ old('marque', $annonce->produit->marque ?? '') }}"
                        >
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input 
                            ...
                            value="{{ old('modele', $annonce->produit->modele ?? '') }}"
                        >
                    </div>
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem;">
                        <div class="form-group">
                            ...
                            <select id="etat" name="etat" class="form-input">
                                ...
                                <option value="Neuf" {{ old('etat', $annonce->produit->etat ?? '') == 'Neuf' ? 'selected' : '' }}>Neuf</option>
                                <option value="Occasion" {{ old('etat', $annonce->produit->etat ?? '') == 'Occasion' ? 'selected' : '' }}>Occasion</option>
                                <option value="Reconditionné" {{ old('etat', $annonce->produit->etat ?? '') == 'Reconditionné' ? 'selected' : '' }}>Reconditionné</option>
                            </select>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input 
                                ...
                                placeholder="1"
                                min="1"
                                value="{{ old('quantite', $annonce->produit->quantite ?? 1) }}"
                            >
                        </div>
                    </div>
                ...
                <!-- Étape 5: Livraison et Disponibilité -->
                <div class="form-step" id="step5">
                    ...
                    <div class="form-instructions">
                        <p class="instruction-text">
                            Indiquez comment vous souhaitez expédier votre produit et sa disponibilité.
                        </p>
                    </div>
                    
                    <div class="form-group">
                        <label for="type_livraison" class="form-label">Moyens d'expédition</label>
                        <select id="type_livraison" name="type_livraison" class="form-input">
                            <option value="">Sélectionnez une option</option>
                            <option value="livraison" {{ old('type_livraison', $annonce->type_livraison) == 'livraison' ? 'selected' : '' }}>Livraison uniquement</option>
                            <option value="retrait" {{ old('type_livraison', $annonce->type_livraison) == 'retrait' ? 'selected' : '' }}>Retrait sur place uniquement</option>
                            <option value="les_deux" {{ old('type_livraison', $annonce->type_livraison) == 'les_deux' ? 'selected' : '' }}>Les deux (livraison et retrait)</option>
                        </select>
                        <small style="color: #666; font-size: 0.875rem; margin-top: 0.5rem; display: block;">
                            Indiquez comment vous souhaitez expédier votre produit.
                        </small>
                    </div>
                    
                    <div class="form-group">
                        <label for="disponibilite" class="form-label">Disponibilité</label>
                        <select id="disponibilite" name="disponibilite" class="form-input">
                            <option value="en_stock" {{ old('disponibilite', $annonce->disponibilite) == 'en_stock' ? 'selected' : '' }}>En stock</option>
                            <option value="rupture_stock" {{ old('disponibilite', $annonce->disponibilite) == 'rupture_stock' ? 'selected' : '' }}>Rupture de stock</option>
                            <option value="sur_commande" {{ old('disponibilite', $annonce->disponibilite) == 'sur_commande' ? 'selected' : '' }}>Sur commande</option>
                        </select>
                    </div>
                ...
                <!-- Étape 6: Options payantes et Publication -->
                <div class="form-step" id="step6">
                    <h1 class="form-title">
                        Options payantes et Publication
                    </h1>
                    
                    <div class="form-instructions">
                        <p class="instruction-text">
                            Choisissez des options pour mettre en avant votre annonce et décidez de sa publication.
                        </p>
                    </div>
                    
                    @if(isset($optionsDisponibles) && count($optionsDisponibles) > 0)
                        <div class="form-group" style="margin-bottom: 2rem;">
                            <label class="form-label">Options payantes (optionnel)</label>
                            <div style="background: #f8f9fa; padding: 1.5rem; border-radius: 8px; margin-top: 0.5rem;">
                                @foreach($optionsDisponibles as $key => $option)
                                    <div style="margin-bottom: 1rem; display: flex; align-items: start; gap: 1rem; padding-bottom: 1rem; border-bottom: 1px solid #e0e0e0;">
                                        <input 
                                            type="checkbox" 
                                            name="options[{{ $key }}]" 
                                            id="option_{{ $key }}" 
                                        ...
                                        >
                                        <div style="flex: 1;">
                                            <label for="option_{{ $key }}" style="color: #333; font-weight: 500; cursor: pointer; display: block; margin-bottom: 0.25rem;">
                                                {{ $option['nom'] }} - {{ number_format($option['prix'], 0, ',', ' ') }} €
                                                @if($option['duree'])
                                                    ({{ $option['duree'] }} jours)
                                                @endif
                                            </label>
                                            <p style="color: #666; font-size: 0.875rem; margin: 0;">{{ $option['description'] }}</p>
                                        </div>
                                    </div>
                                @endforeach
                            </div>
                    ...
                    <div class="form-group">
                        ...
                        <select id="statut" name="statut" class="form-input">
                            <option value="brouillon" {{ old('statut', $annonce->statut) == 'brouillon' ? 'selected' : '' }}>Enregistrer en brouillon</option>
                            <option value="publiee" {{ old('statut', $annonce->statut) == 'publiee' ? 'selected' : '' }}>Publier immédiatement</option>
                        </select>
                        <small style="color: #666; font-size: 0.875rem; margin-top: 0.5rem; display: block;">
                            Vous pourrez publier votre annonce plus tard depuis votre tableau de bord.
                        </small>
                    </div>
    ...
    <script>
        ...
        function validateCurrentStep() {
            ...
            if (currentStep === 1) {
                ...
                const hasNewImages = (typeof uploadedImages !== 'undefined' && uploadedImages.length > 0) || 
                                    (photosInput && photosInput.files && photosInput.files.length > 0);
                ...
                const totalFiles = (typeof uploadedImages !== 'undefined' ? uploadedImages.length : 0) + 
                                 (photosInput && photosInput.files ? photosInput.files.length : 0);
        ...
        let photosInput, imagePreviewContainer, imageUploadArea;
        let deletedMediaIds = [];
        ...
        function renderImagePreview() {
            ...
            uploadedImages.forEach((imageData, index) => {
                ...
                previewItem.className = `image-preview-item ${index === mainImageIndex ? 'main' : ''}`;
                previewItem.innerHTML = `
                    ...
                    <div class="image-preview-actions">
                        ...
                        <button type="button" class="image-preview-btn" onclick="removeImage(${index})" title="Supprimer">×</button>
                    </div>
