...
@section('content')
    ...
    <div class="dashboard-container">
        ...
        <main class="main-content">
            ...
            <div
                style="background: linear-gradient(135deg, #004aad 0%, #002a6b 100%); padding: 1.4rem 1.75rem; border-radius: 16px; margin-bottom: 1.75rem; margin-left: auto; border: 1px solid rgba(255,255,255,0.15); box-shadow: 0 10px 20px -10px rgba(0,0,0,0.2); position: relative; overflow: hidden; color: white; max-width: 800px; isolation: isolate;">
                ...
                <div style="margin-bottom: 2rem;">
                    ...
                    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 1.25rem;">
                        @foreach($packs as $pack)
                            @php
                                $isPopular = ($pack->nom ?? $pack->label ?? '') === 'Pack 25 000' || ($pack->popular ?? false) || ($pack->credits ?? $pack['credits'] ?? 0) == 100;
                            @endphp
                            <div
                                style="background: #f8f9fa; padding: 1rem; border-radius: 8px; border: 1px solid {{ $isPopular ? '#EF3B2D' : '#dee2e6' }}; position: relative; display: flex; flex-direction: column; justify-content: space-between;">
                                ...
                                <form action="{{ route('account.credits.checkout') }}" method="POST">
                                    ...
                                    <button type="submit"
                                        style="display: block; width: 100%; text-align: center; background: {{ ($loop->first || $loop->last) ? '#004aad' : '#f68b1e' }}; color: white; padding: 0.6rem; border-radius: 6px; border: none; cursor: pointer; font-size: 0.85rem; font-weight: 700;">
                                        Acheter
                                    </button>
                                </form>
                            </div>
