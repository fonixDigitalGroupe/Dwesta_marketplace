...
@unless(request()->routeIs('admin.messagerie.*'))
    ...
    @if($awCount > 0 && $awLatest)
        ...
        <style>
            ...
            #aw-widget .aw-head { display: flex; align-items: center; gap: 10px; padding: 12px 14px; background: #f68b1e; color: #fff; }
            #aw-widget .aw-icon { width: 36px; height: 36px; border-radius: 50%; background: rgba(255,255,255,0.2); display: flex; align-items: center; justify-content: center; flex-shrink: 0; position: relative; }
            ...
            #aw-widget .aw-title { font-weight: 700; font-size: 0.9rem; flex: 1; }
            #aw-widget .aw-close { cursor: pointer; color: #fff; background: none; border: none; font-size: 1.1rem; }
